var VisitModel= (function () {
    ...
    var increment_attemp_gps_alert= function(visit){
        var newVal= visit.attemp_gps_alert===null?1:visit.attemp_gps_alert;
        newVal['attemp_gps_alert']= attemp_gps_alert;
        update({id: visit.id}, newVal);
    };
    ...
    var update_distances_in_mtrs= function(current_position, callback){
        ...
        findRaw('lat is not null and long is not null', {success: function(tx, results){
            $.each(results._all, function(index, visit){
                var distance= Haversine.distance(current_position, visit);
                distance= distance | 0;
                updates.push(
                    "UPDATE "+table+" SET distance_in_mts = "+distance+" where id="+visit.id
                );
            });
